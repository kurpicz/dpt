...
class local_test : public ::testing::Test {
...
TEST_F(local_test, RequestCharacter) {
  ...
  result = dpt::com::local_communication<char, uint32_t, uint32_t>::
    request_characters(request_positions, part_);
  for (const auto& rec_char : result) {
    ASSERT_EQ('b', rec_char);
  }
...
TEST_F(local_test, RequestSubstring) {
  std::vector<uint32_t> request_positions;
...
TEST_F(local_test, RequestSubstrinHead) {
  std::vector<uint32_t> request_positions;
  std::vector<uint32_t> request_lengths;
  for (int32_t i = 0; i < env_.size(); ++i) {
    request_positions.emplace_back(1);
  ...
  std::tie(heads, tails) =
    dpt::com::local_communication<char, uint32_t, uint32_t>::
